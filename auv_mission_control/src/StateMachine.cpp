...
ROS_INFO("Initial Yaw %f", pm.getYaw());

switch(currentState){
  case 0: {//init
    pm.setPidEnabled(AXIS_HEAVE, false);
    pm.setPidEnabled(AXIS_SWAY, false);
    pm.setPidEnabled(AXIS_SURGE, false);
    //pm.setPidEnabled(AXIS_YAW, false);
    pm.setControlEffort(AXIS_HEAVE, 0);
    pm.setControlEffort(AXIS_SWAY, 0);
    pm.setControlEffort(AXIS_SURGE, 0);
    //pm.setControlEffot(AXIS_HEAVE, 0);
    if(initCount < 1){
