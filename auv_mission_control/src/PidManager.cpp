...
//  imu_ = *imu_msg;
  ...
  //changed on August 4th, trying to find initial yaw value.
...
//
  ...
  subDepth = nh_.subscribe("/state_heave", 20, &PidManager::depthCallBack, this);
  ...
  //plant state publishers
   ...
   pubStateHeave = nh_.advertise<std_msgs::Float64>("state_heave_not", 10);
...
void PidManager::setZero(int sensor){
  if(subImuHasBeenCalled){
    ...
    ROS_INFO("Yaw init Value %f", yawInitValue);	
}
}
